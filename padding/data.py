...
def read_sentences(datafile):
    ...
    # normalize
    data = [prepare_string(l) for l in lines]
...
def prepare_data(datafile):
    ...
    for datum in data:
        if len(datum) <= MAX_LEN:
            newdatum = pad(datum, MAX_LEN)
            vocab.index_words(newdatum)
            if random.random() < test_to_train:
                ret_data[1].append(newdatum)
            else:
                ret_data[0].append(newdatum)
